...
static void print_active_wakeup_sources(void)
{
	...
	list_for_each_entry_rcu(ws, &wakeup_sources, entry) {
		if (ws->active) {
			if (strstr(ws->name, "EINT"))
				pr_info("active wakeup source: %s (irq:%d)\n", ws->name, get_current_eint());
			else
	...
	if (!active && last_activity_ws) {
		if (strstr(last_activity_ws->name, "EINT"))
			pr_info("last active wakeup source: %s(irq:%d)\n",
				last_activity_ws->name, get_current_eint());
		else
