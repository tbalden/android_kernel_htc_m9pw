...
int htc_is_need_sw_reboot(void) {
	...
	if (!htc_reboot_info.save_emmc_support.is_saved_to_emmc) {
		ret = 1;
	}
...
static int htc_reboot_info_save_emmc(char* cmd) {
	...
	htc_reboot_info.save_emmc_support.is_saved_to_emmc = 0;
	...
	htc_reboot_info.save_emmc_support.is_saved_to_emmc = 1;
