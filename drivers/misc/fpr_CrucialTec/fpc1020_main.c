...
#define FPC1020_INTERRUPT_WORKER_NAME		"fpc1020_interrupt_worker"
...
static void fpc1020_interrupt_worker_funtion(struct work_struct *work);

static int fpc1020_update_finger_detect_setting(fpc1020_data_t *fpc1020);
		...
		const char *buf, size_t count)
{
	...
	{
		case 0:
			printk("[fp][CT]sleep_store: leave deep sleep mode\n");
			fpc1020_sleep(fpc1020_temp, false);
			break;
		case 1:
			printk("[fp][CT]sleep_store: enter deep sleep mode\n");
			fpc1020_sleep(fpc1020_temp, true);
			break;
		default:
			printk("[fp][CT]keyevent_store: not support item\n");
			break;
	}
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	switch(enable)
	{
		case 0:
			htc_disable_clk(0x1);
			printk("[fp][CT]clk_store: disable SPI clock\n");
			break;
		case 1:
			htc_enable_clk(0x1);
			printk("[fp][CT]clk_store: enable SPI clock\n");
			break;
		default:
			printk("[fp][CT]clk_store: not support item\n");
			break;
	}
	return count;
}
		...
		char *buf)
{
	...
	if(toggle%2 == 0)
	{
		fp_clk_ctrl = 1;
		htc_enable_clk(0x1);
		printk("[fp][CT]clk_show: enable SPI clock --%d\n", toggle);
	}
	else
	{
		msleep(1000);
		...
		htc_disable_clk(0x1);
		fp_clk_ctrl = 0;
		printk("[fp][CT]clk_show: disable SPI clock --%d\n", toggle);
	}
	toggle++;
	return 0;
}
...
static ssize_t fp_mode_store(struct device *dev, struct device_attribute *attr,
    const char *buf, size_t count)
{
    unsigned long tmp;
    int error;
    ...
    if (error) {
        ...
        if (tmp >= FP_MODE_MAX){
            pr_err("%s invalid fp_mode:%lu\n", __func__, tmp);
        }
        else {
            fpc1020_temp->fp_mode = tmp;
            pr_debug("%s fp_mode:%d\n", __func__, fpc1020_temp->fp_mode );
            fpc1020_update_finger_detect_setting(fpc1020_temp);
        }
    }
...
static ssize_t fp_mode_show(struct device *dev, struct device_attribute *attr,
    char *buf)
{
    pr_info("%s fp_mode:%d\n", __func__, fpc1020_temp->fp_mode );
    return snprintf(buf, PAGE_SIZE, "%d\n", fpc1020_temp->fp_mode);
}
...
static DEVICE_ATTR(clk, S_IRUGO | S_IWUSR , clk_show, clk_store);
static DEVICE_ATTR(sleep, S_IRUGO | S_IWUSR , NULL, sleep_store);
static DEVICE_ATTR(fp_mode, S_IRUGO | S_IWUSR , fp_mode_show, fp_mode_store);
...
#endif
	if(fpc1020_firmware != NULL)
	printk(KERN_INFO "%s : fp_clk_ctrl = %d, file_opened = %d, req_mode = %d\n",__func__, fp_clk_ctrl, fpc1020_temp->file_opened, fpc1020_temp->worker.req_mode);

	if (fpc1020_firmware != NULL && fpc1020_temp->worker.req_mode == FPC1020_WORKER_IDLE_MODE && !fpc1020_temp->file_opened && fp_clk_ctrl != 1){
		fpc1020_input_enable(fpc1020_temp,true);
		fpc1020_start_input(fpc1020_temp);
	}
...
static int fpc1020_probe(struct spi_device *spi)
{
	...
	wake_lock_init(&fpc1020->report_wake_lock, WAKE_LOCK_SUSPEND, "fp_report_wake_lock");
	printk("[fp][CT]Fingerprint wakelock init\n");
	...
	fpc1020->file_opened = false;
	fpc1020->screen_state = true;
	fpc1020->fp_mode = FP_MODE_NORMAL;
	fpc1020->charger_enabled = true;

	init_waitqueue_head(&fpc1020->wq_irq_return);
#ifdef INTERRUPT_WORKER
	INIT_WORK(&fpc1020->interrupt_work, fpc1020_interrupt_worker_funtion);
	fpc1020->workqueue = create_singlethread_workqueue(FPC1020_INTERRUPT_WORKER_NAME);
#endif
        if (error)
                goto err;
	...
	fpc1020_update_finger_detect_setting(fpc1020);
...
static int fpc1020_suspend(struct device *dev)
{
	...
	dev_dbg(&fpc1020->spi->dev, "%s\n", __func__);
        ...
        if (fpc1020->worker.req_mode != FPC1020_WORKER_CAPTURE_MODE && !fpc1020->file_opened && fp_clk_ctrl != 1)
	{
...
static int fpc1020_release(struct inode *inode, struct file *file)
{
	...
	if(!fpc1020->charger_enabled) {
		dev_dbg(&fpc1020->spi->dev,"Enable USB Charger\n");
		charger_switch_internal(ENABLE_PWRSRC_FINGERPRINT);
		fpc1020->charger_enabled = true;
	}
...
#endif
		if (fpc1020_firmware != NULL && fpc1020->worker.req_mode == FPC1020_WORKER_IDLE_MODE && !fpc1020->file_opened && fp_clk_ctrl != 1){

#ifdef INTERRUPT_WORKER
			queue_work(fpc1020->workqueue, &fpc1020->interrupt_work);
#else
			fpc1020_input_enable(fpc1020,true);
			fpc1020_start_input(fpc1020);
#endif
...
static int fpc1020_update_finger_detect_setting(fpc1020_data_t *fpc1020)
{
    if (!fpc1020->screen_state) {
        fpc1020->setup.capture_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_VERIFY;
        fpc1020->setup.input_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_VERIFY;
    }
    else {
        switch (fpc1020->fp_mode) {
        case FP_MODE_NORMAL:
            fpc1020->setup.capture_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_HOME;
            fpc1020->setup.input_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_HOME;
            break;

        case FP_MODE_ENROLL:
            fpc1020->setup.capture_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_ENROLL;
            fpc1020->setup.input_finger_down_threshold = HTC_FINGER_DOWN_THRESHOLD_ENROLL;
            break;
    ...
    pr_info("%s fp_mode:%d, screen:%d, threshold:%d\n", __func__, fpc1020_temp->fp_mode, fpc1020->screen_state, fpc1020->setup.input_finger_down_threshold);
