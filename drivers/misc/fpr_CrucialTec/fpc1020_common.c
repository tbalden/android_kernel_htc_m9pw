...
const fpc1020_setup_t fpc1020_setup_default_1150_a1b1f1 = {
	...
	.input_finger_down_threshold	= 7,
	.finger_detect_threshold	= 32,
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
	...
	while (1) {
		...
		if(fpc1020->finger_state == FPC1020_FINGER_STATE_DOWN)
	                error = fpc1020_wait_for_irq(fpc1020,
                                                FPC1020_WAIT_FINGER_PRESENT_TIMEOUT_MS);
		else
			error = fpc1020_wait_for_irq(fpc1020,
						FPC1020_DEFAULT_IRQ_TIMEOUT_MS);
		...
		if (error < 0) {
			...
			if (error != -ETIMEDOUT)
				return error;
			if (error == -ETIMEDOUT && fpc1020->worker.req_mode == 2)
				return error;
		}
	}
}
...
int fpc1020_check_finger_present_sum(fpc1020_data_t *fpc1020)
{
	...
	if (zones < 0) {
		dev_dbg(&fpc1020->spi->dev, "%s zones:%d\n", __func__, zones);
		return zones;
	}
	else {
		zones &= mask;
