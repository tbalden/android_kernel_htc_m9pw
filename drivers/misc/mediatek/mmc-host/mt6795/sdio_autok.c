...
#define AUTOK_CKGEN_ALLOW_MAX           (3) 
    ...
    P_AUTOK_CMD_TUNE_RES_T pCmdTuneRes)
{
    ...
    unsigned int baseCKGEN = g_ckgen_allow_start;
    #else
    ...
    #if DYNAMIC_CKGEN_RANGE_SEL_EN  
	  unsigned int CKGENMax =g_ckgen_allow_end;
	  #else
    ...
    if (stg == TUNING_STG1) {
        ...
        #if DYNAMIC_CKGEN_RANGE_SEL_EN        
        range_max = MAX_GET(range_max,g_ckgen_allow_end); 
        #endif
            ...
            if(RealCKGEN < baseCKGEN){
                ...
                AUTOK_PRINT("RBound:%d, LBound:%d, CKGEN shift Mar:%d\r\n", 
                    RBoundPad, LBound, NewMargin);
							if ((NewMargin>0)&&(NewMargin/2 > (int)0) && (NewMargin/2 >= (int)(RBoundPad + 2))) {   
...
static void autok_vcore_set(unsigned int vcore_uv) 
{
    ...
    if(vcore1_reg != 0){
        ...
        if( sdio_vio18_flag > 1600 && sdio_vio18_flag <2000)
        {
            ...
            pmic_config_interface(0x396, vio18_reg, 0x7F, 0); 
	...
	if( sdio_vcore1_flag > 900 && sdio_vcore1_flag <1200)
	{
            ...
            pmic_config_interface(0x24c, vcore1_reg, 0x7F, 0); 
	...
	if( sdio_vcore2_flag > 900 && sdio_vcore2_flag <1200)
	{
            ...
            pmic_config_interface(0x36a, vcore2_reg, 0x7F, 0); 
