...
static void mtk_spi_workq(struct work_struct *work)
{
	printk("%s +++\n",__func__);

	unsigned long           flags;
	struct mt_spi_t *ms = container_of(work, struct mt_spi_t, work);

	spin_lock_irqsave(&ms->lock,flags);

	struct spi_message *msg =
		list_entry(ms->queue.next, struct spi_message, queue);

	if(unlikely(!msg)){
		pr_err("msg is invalid \n");
		goto out;
	}

        if(unlikely(!msg->state)){
		pr_err("msg config is invalid \n");
                goto out;
        }

        if(unlikely(!is_interrupt_enable(ms))){
		pr_err("interrupt is disable\n");
		goto out;
        }
	...
	if(unlikely(!xfer)){
		pr_err("xfer is invalid\n");
		goto out;
	}

	if(is_last_xfer(msg, xfer)){
		mt_spi_msg_done(ms, msg, -ETIMEDOUT);
	}else{
...
#if 1
int htc_spi_vote = 0;
spinlock_t htc_spi_lock;

void htc_enable_clk(int bit)
{
#if (!defined(CONFIG_MT_SPI_FPGA_ENABLE))
	unsigned long flags;
	spin_lock_irqsave(&htc_spi_lock, flags);
	if(htc_spi_vote == 0 && (htc_spi_vote | bit != 0))
	{
		
		enable_clock(MT_CG_PERI_SPI0, "spi");
	}
	htc_spi_vote = htc_spi_vote | bit;
	spin_unlock_irqrestore(&htc_spi_lock, flags);
#endif
	return;
}

void htc_disable_clk(int bit)
{
#if (!defined(CONFIG_MT_SPI_FPGA_ENABLE))
	unsigned long flags;
	spin_lock_irqsave(&htc_spi_lock, flags);
	int pre_vote = htc_spi_vote;
	htc_spi_vote = htc_spi_vote&(~bit);
	if((pre_vote != 0) && (htc_spi_vote == 0))
	{
...
mt_spi_next_xfer(struct mt_spi_t *ms, struct spi_message *msg)
{
	...
	if(unlikely(!ms)){
		pr_err("master wrapper is invalid \n");
		ret = -EINVAL;
		return ret;
	}
	if(unlikely(!msg)){
	...
	queue_delayed_work(ms->workqueue, &ms->work, msecs_to_jiffies(2000));
...
static int mt_spi_setup ( struct spi_device *spidev )
{
	...
	if ( !chip_config ) {
		...
		chip_config->high_time = 7;
		chip_config->low_time = 7;
...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&ms->work, mtk_spi_workq);
	ms->workqueue = create_singlethread_workqueue(dev_name(&pdev->dev));
	if (!ms->workqueue)
		goto out;
...
#ifdef CONFIG_PM
static int spi_suspend_gpio_config(struct platform_device *pdev)
{
    dev_info(&pdev->dev,"[R] %s\n", __func__ );

    #ifdef GPIO_SPI_CS_1_PIN
    mt_set_gpio_pull_select(GPIO_SPI_CS_1_PIN, GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_SPI_CS_1_PIN, TRUE);
    mt_set_gpio_mode(GPIO_SPI_CS_1_PIN, GPIO_SPI_CS_1_PIN_M_GPIO);
    #endif 

    #ifdef GPIO_SPI_CS_0_PIN
    mt_set_gpio_pull_select(GPIO_SPI_CS_0_PIN, GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_SPI_CS_0_PIN, TRUE);
    mt_set_gpio_mode(GPIO_SPI_CS_0_PIN, GPIO_SPI_CS_0_PIN_M_GPIO);
    #endif 

    #ifdef GPIO_SPI_SCK_PIN
    mt_set_gpio_pull_select(GPIO_SPI_SCK_PIN, GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_SPI_SCK_PIN, TRUE);
    mt_set_gpio_mode(GPIO_SPI_SCK_PIN, GPIO_SPI_SCK_PIN_M_GPIO);
    #endif 

    #ifdef GPIO_SPI_MISO_PIN
    mt_set_gpio_pull_select(GPIO_SPI_MISO_PIN, GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_SPI_MISO_PIN, TRUE);
    mt_set_gpio_mode(GPIO_SPI_MISO_PIN, GPIO_SPI_MISO_PIN_M_GPIO);
    #endif 

    #ifdef GPIO_SPI_MOSI_PIN
    mt_set_gpio_pull_select(GPIO_SPI_MOSI_PIN, GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_SPI_MOSI_PIN, TRUE);
...
static int spi_resume_gpio_config(struct platform_device *pdev)
{
    dev_info(&pdev->dev,"[R] %s\n", __func__ );

    #ifdef GPIO_SPI_CS_1_PIN
    mt_set_gpio_mode(GPIO_SPI_CS_1_PIN, GPIO_SPI_CS_1_PIN_M_SPI_CS);
    #endif 

    #ifdef GPIO_SPI_CS_0_PIN
    mt_set_gpio_mode(GPIO_SPI_CS_0_PIN, GPIO_SPI_CS_0_PIN_M_SPI_CS);
    #endif 
...
{
	spi_suspend_gpio_config(pdev);
	return 0;	
}
static int mt_spi_resume ( struct platform_device *pdev )
{
	spi_resume_gpio_config(pdev);
	return 0;
}
#else
