...
enum {
	MT_FH_DEBUG_FH_CONFIG = BIT(0),
};

static int mt_fh_debug_mask = 0;
module_param_named(
	debug_mask, mt_fh_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
int freqhopping_config(unsigned int pll_id, unsigned long vco_freq, unsigned int enable)
{
	...
	if (mt_fh_debug_mask & MT_FH_DEBUG_FH_CONFIG)
