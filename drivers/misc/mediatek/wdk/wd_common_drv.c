...
#define msgwdk(msg...) printk(KERN_DEBUG PFX msg);
...
static int wk_proc_cmd_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	msgwdk("[WDK] mode=%d interval=%d timeout=%d enable =%d\n", mode, kinterval, timeout, en);
	...
	if (1 == en) {
		...
		msgwdk("[WDK] enable local wdt\n");
#endif
		msgwdk("[WDK] enable wdt\n");
	}
	if (0 == en) {
		mtk_wdt_enable(WK_WDT_DIS);
#ifdef CONFIG_LOCAL_WDT
		local_wdt_enable(WK_WDT_DIS);
		msgwdk("[WDK] disable local wdt\n");
#endif
		msgwdk("[WDK] disable wdt\n");
	}
	...
	if (1 == mode) {
		...
		msgwdk("[WDK] use irq mod\n");
	} else if (0 == mode) {
		
		mtk_wdt_swsysret_config(0x20000000, 0);
		msgwdk("[WDK] use reboot mod\n");
	} else if (2 == mode) {
...
static int start_kicker_thread_with_default_setting(void)
{
	...
	msgwdk("[WDK] fwq start_kicker_thread_with_default_setting done\n");
	return ret;
}
...
void wk_start_kick_cpu(int cpu)
{
	if (IS_ERR(wk_tsk[cpu])) {
		msgwdk("[wdk]wk_task[%d] is NULL\n", cpu);
	} else {
		kthread_bind(wk_tsk[cpu], cpu);
		msgwdk("[wdk]bind thread[%d] to cpu[%d]\n", wk_tsk[cpu]->pid, cpu);
...
void kicker_cpu_bind(int cpu)
{
	...
	else
	{
		...
		msgwdk("[wdk]bind kicker thread[%d] to cpu[%d]\n",wk_tsk[cpu]->pid,cpu);
		wake_up_process(wk_tsk[cpu]);
	}
}
...
static int kwdt_thread(void *arg)
{
	...
	msgwdk("[WDK] WDT kicker thread stop, cpu:%d, pid:%d\n", cpu, current->pid);
	return 0;
}
...
static int start_kicker(void)
{
	...
	g_kicker_init = 1;
	msgwdk("[WDK] WDT start kicker  done\n");
	return 0;
}
...
static int __cpuinit wk_cpu_callback(struct notifier_block *nfb, unsigned long action, void *hcpu)
{
	...
	case CPU_ONLINE_FROZEN:
		...
		mtk_wdt_restart(WD_TYPE_NORMAL);	
#ifdef CONFIG_LOCAL_WDT
		msgwdk("[WDK]cpu %d plug on kick local wdt\n", hotcpu);
		...
		htc_debug_watchdog_update_last_pet(sched_clock());
#endif
		msgwdk("[WDK]cpu %d plug on kick wdt\n", hotcpu);
		break;
	...
	case CPU_DEAD_FROZEN:
		...
		mtk_wdt_restart(WD_TYPE_NORMAL);	
#ifdef CONFIG_LOCAL_WDT
		msgwdk("[WDK]cpu %d plug off kick local wdt\n", hotcpu);
		...
		htc_debug_watchdog_update_last_pet(sched_clock());
#endif
		msgwdk("[WDK]cpu %d plug off, kick wdt\n", hotcpu);
		break;
#endif				
	}
...
static int __init init_wk(void)
{
	...
	for (i = 0; i < CPU_NR; i++) 
	{
		if(cpu_online(i))
		{
			...
			msgwdk("[WDK]init cpu online %d\n",i);
		}
		else
		{
			wk_cpu_update_bit_flag(i, 0);
			msgwdk("[WDK]init cpu offline %d\n",i);
		}
	}
	mtk_wdt_restart(WD_TYPE_NORMAL);	
	cpu_hotplug_enable();
	msgwdk("[WDK]init_wk done late_initcall cpus_kick_bit=0x%x -----\n", cpus_kick_bit);
...
{
	int i = 0;
	msgwdk("[WDK]arch init check_bit=0x%x+++++\n", cpus_kick_bit);
	for (i = 0; i < CPU_NR; i++) {
