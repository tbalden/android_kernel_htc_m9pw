...
static struct LCM_setting_table lcm_initialization_setting[] =
{
	...
	{0xB9, 7, {0x67, 0x45, 0x15, 0x40, 0x09, 0x00, 0x00}},
	{0xBA, 7, {0xCD, 0x55, 0x15, 0x40, 0x09, 0x00, 0x00}},
...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->pwm_min = 5;
	params->pwm_default = 60;
	params->pwm_max = 213;
	params->camera_blk = 191;
	params->camera_dua_blk = 191;
	params->dim_cmd = 0x2c;
}
...
static unsigned int lcm_check_id(void)
{
	...
	if (pcbid == XA || pcbid == XB || pcbid == XC) {
		LCD_LOG("[DISP] %s: JDI pcbid %d, engid %X\n", __func__, pcbid, htc_setting_engineerid());
		return (htc_setting_engineerid() == 0x0) ? 1 : 0;
	} else {
		LCD_LOG("[DISP] %s: JDI pcbid %d, triple %d\n", __func__, pcbid, which_lcd_module_triple());
		return (which_lcd_module_triple() == 0x0) ? 1 : 0;
	}
#else
	return 0;
#endif
