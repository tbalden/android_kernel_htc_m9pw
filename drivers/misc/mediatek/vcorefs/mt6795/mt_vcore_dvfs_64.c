...
#define FDDR_S3_KHZ		800000		

#define FAXI_S1_KHZ		273000		
#define FAXI_S2_KHZ		218400		

#define FMM_S1_KHZ		400000		
#define FMM_S2_KHZ		317000		

#define FVENC_S1_KHZ		494000		
#define FVENC_S2_KHZ		324000		

#define FVDEC_S1_KHZ		494000		
#define FVDEC_S2_KHZ		324000		

#define DVFS_CMD_SETTLE_US	5	
...
static void __update_vcore_pdn(struct pwr_ctrl *pwrctrl, int steps)
{
	...
	if (pwrctrl->dma_dummy_read) {
		...
		dma_dummy_read_for_vcorefs(loops);	
	} else {
...
unsigned long htc_vcore_vol_get(void)
{
	return vcore_pmic_to_uv(get_vcore_pdn());
}

int htc_vcore_vol_set(int vcore_uv)
{
	struct pwr_ctrl *pwrctrl = &vcorefs_ctrl;
	struct dvfs_opp *opp;
	int steps = 0;
	...
	steps = vcore_uv_to_pmic(vcore_uv);
	if (steps >= VCORE_INVALID)
		return -EBUSY;
