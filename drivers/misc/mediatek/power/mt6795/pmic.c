...
extern kal_uint32 charging_oc_init(void);
extern kal_uint32 charging_current_level_up(void);
extern kal_uint32 charging_current_level_drop(void);
#endif 
...
#endif
#ifdef CONFIG_MTK_SWCHR_SUPPORT
static kal_uint32 pre_drop_time = 0x12345678;
static kal_uint32 duration_time = 0x12345678;
static kal_uint32 pre_up_time = 0x12345678;
static kal_uint32 duration_up_time = 0x12345678;
#endif
...
extern kal_int32 current_oc_level;
...
#define BAT_HV_THD   0x880 
#define BAT_LV_1_THD 0x820 
#define BAT_LV_2_THD 0x780 
#define BAT_LV_3_THD 0x700 
...
#define BAT_OC_H_THD   65535-((250334*4)/CAR_TUNE_VALUE)
#define BAT_OC_L_THD   65535-((250334*11/2)/CAR_TUNE_VALUE)
...
int bat_percent_notify_handler(void *unused)
{
    ...
    do
    {
        ...
        xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "bat_per_level=%d,bat_per_val=%d\n",g_battery_percent_level,bat_per_val);
...
enum hrtimer_restart bat_percent_notify_task(struct hrtimer *timer)
{
    ...
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "bat_percent_notify_task is called\n");
...
{
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[wake_up_pmic_mt6331]\r\n");
...
{
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[wake_up_pmic_mt6332]\r\n");
...
void cust_pmic_interrupt_en_setting_mt6331(void)
{
    ...
    mt6331_upmu_set_rg_int_en_ldo_oc(1);
    
    mt6331_upmu_set_rg_vmch_ocfb_en(1);
}
...
void cust_pmic_interrupt_en_setting_mt6332(void)
{
    ...
    mt6332_upmu_set_rg_int_en_otg_oc(1);
...
void pwrkey_int_handler(void)
{
    ...
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[pwrkey_int_handler]....\n");
...
void homekey_int_handler(void)
{
    ...
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[homekey_int_handler]....\n");
...
void mt6331_ldo_oc_int_handler(void)
{
    ...
    pmic_read_interface((kal_uint32)(MT6331_OCSTATUS0), &reg_val, 0xFFFF, 0x0);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[mt6331_ldo_oc_int_handler] MT6331_OCSTATUS0=0x%x\n", reg_val);

    pmic_read_interface((kal_uint32)(MT6331_OCSTATUS1), &reg_val, 0xFFFF, 0x0);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[mt6331_ldo_oc_int_handler] MT6331_OCSTATUS1=0x%x\n", reg_val);

    pmic_read_interface((kal_uint32)(MT6331_OCSTATUS2), &reg_val, 0xFFFF, 0x0);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[mt6331_ldo_oc_int_handler] MT6331_OCSTATUS2=0x%x\n", reg_val);

    
    if(1 == mt6331_upmu_get_oc_status_vmch())
       msdc_sd_power_off();
...
#ifdef CONFIG_MTK_PMIC_DVT_SUPPORT
extern void mt6332_bat_int_close(void);
#endif

void mt6332_bat_int_check_setting(kal_int32 temp)
{
#ifdef LOW_BATTERY_PROTECT
    if( g_low_battery_level == 1 ){
        if( temp >= 0 )
            ...
            mt6332_upmu_set_auxadc_lbat_volt_min(BAT_LV_3_THD);
        }
    ...
    xlog_printk(ANDROID_LOG_INFO, "mt6332_bat_int_check_setting", "Reg[0x%x]=0x%x\n",
        MT6332_AUXADC_CON18, upmu_get_reg_value(MT6332_AUXADC_CON18));
#endif
}
...
void mt6332_bat_h_int_handler(void)
{
    kal_uint32 ret=0;
#ifdef LOW_BATTERY_PROTECT
    kal_int32 iTemp = BMT_status.temperature_now;
#endif
...
void mt6332_bat_l_int_handler(void)
{
    ...
    if(g_low_battery_level==1)      
        ...
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[mt6332_bat_l_int_handler] hit criticall low.\n");
...
{
    kal_uint32 ret=0;
	kal_uint32 drop_time = 0;
	kal_uint32 ret_data = 0;

#ifdef BATTERY_OC_PROTECT
#ifdef CONFIG_MTK_SWCHR_SUPPORT
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[fg_cur_h_int_handler]....%d\n", g_fg_is_charging);

	if (g_fg_is_charging) {
		bat_oc_h_en_setting(0);
		bat_oc_l_en_setting(0);
		drop_time = sched_clock();
		if (pre_drop_time == 0x12345678) {
			pre_drop_time = sched_clock();
		} else {
			duration_time += abs(drop_time - pre_drop_time);	
		}

		battery_xlog_printk(BAT_LOG_CRTI,"[fg_cur_h_int_handler] duration_timel=%d\n", duration_time);
		ret=pmic_config_interface(MT6332_INT_STATUS2,0x1,0x1,11);
		if (duration_time > 10000000) {
			fgauge_read_current(&ret_data);
			
			while (ret_data > CUST_BAT_OC_H_THD*10) {
...
void fg_cur_l_int_handler(void)
{
	...
	if (g_fg_is_charging) {
		...
		bat_oc_l_en_setting(0);
		up_time = sched_clock();
		if (pre_up_time == 0x12345678) {
			pre_up_time = sched_clock();
		} else {
			duration_up_time += abs(up_time - pre_up_time);	
		}

		battery_xlog_printk(BAT_LOG_CRTI,"[fg_cur_l_int_handler] duration_timel=%d\n", duration_up_time);
		ret=pmic_config_interface(MT6332_INT_STATUS2,0x1,0x1,12);
		if (duration_up_time > 10000000) {
			charging_current_level_up();
			duration_up_time = 0;
		}
		mdelay(1);
		pre_up_time = sched_clock();
		battery_xlog_printk(BAT_LOG_CRTI,"[fg_cur_l_int_handler] pre_drop_time=%d\n", pre_up_time);
		bat_oc_h_en_setting(1);
		bat_oc_l_en_setting(1);
	} else 
#endif
...
static int pmic_thread_kthread_mt6331(void *x)
{
    ...
    while (1) {
        ...
        xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[PMIC31_INT] pwrap_eint_status=0x%x\n", pwrap_eint_status);

        mt6331_int_handler();
        
        pmic_wrap_eint_clr(0x0);
        xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[PMIC31_INT] pmic_wrap_eint_clr(0x0);\n");
        ...
        xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[PMIC31_INT] after ,mt6331_int_status_val_0=0x%x\n", mt6331_int_status_val_0);
        xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[PMIC31_INT] after ,mt6331_int_status_val_1=0x%x\n", mt6331_int_status_val_1);   
...
{
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[mt_pmic_eint_irq_mt6331] receive interrupt\n");
    wake_up_pmic_mt6331();
    return ;
}
...
{
    xlog_printk(ANDROID_LOG_VERBOSE, "Power/PMIC", "[mt_pmic_eint_irq_mt6332] receive interrupt\n");
    wake_up_pmic_mt6332();
    return ;
}
...
static ssize_t show_LDO_VSIM1_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    else if(reg_val == 4)    ret_value = 1860;
    else if(reg_val == 5)    ret_value = 2760;
    else if(reg_val == 6)    ret_value = 3000;
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VDVFS11_enable] %d\n", dctEnable);
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VDVFS12_enable] %d\n", dctEnable);
...
void dct_pmic_VDVFS13_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VDVFS13_enable] %d\n", dctEnable);
...
void dct_pmic_VDVFS14_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VDVFS14_enable] %d\n", dctEnable);
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VCORE2_enable] %d\n", dctEnable);
...
void dct_pmic_MT6332_VDVFS2_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VDVFS2_enable] %d\n", dctEnable);
...
void dct_pmic_MT6332_VDRAM_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VDRAM_enable] %d\n", dctEnable);

	if(dctEnable == KAL_TRUE) {
		mt6332_upmu_set_vdram_oc_en(1);
	} else {
		mt6332_upmu_set_vdram_oc_en(0);
	}
}

void dct_pmic_MT6332_VRF1_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VRF1_enable] %d\n", dctEnable);
...
void dct_pmic_MT6332_VRF2_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VRF2_enable] %d\n", dctEnable);
...
void dct_pmic_MT6332_VPA_enable(kal_bool dctEnable)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_MT6332_VPA_enable] %d\n", dctEnable);
...
void dct_pmic_VGP2_sel(kal_uint32 volt)
{
    ...
    else
    {
        ...
        pmic_config_interface(0x546,0x1,0x1,4); 
...
void dct_pmic_VDVFS11_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS11_sel] value=%d \n", volt);

    if (volt > 1310000 || volt < 700000)
        return;
    ...
    mt6331_upmu_set_vdvfs11_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS11_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_VDVFS12_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS12_sel] value=%d \n", volt);
    ...
    mt6331_upmu_set_vdvfs12_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS12_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_VDVFS13_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS13_sel] value=%d \n", volt);
    ...
    mt6331_upmu_set_vdvfs13_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS13_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_VDVFS14_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS14_sel] value=%d \n", volt);
    ...
    mt6331_upmu_set_vdvfs14_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VDVFS14_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_VCORE2_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VCORE2_sel] value=%d \n", volt);
    ...
    mt6331_upmu_set_vcore2_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_VCORE2_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_MT6332_VDVFS2_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VDVFS2_sel] value=%d \n", volt);
    ...
    mt6332_upmu_set_vdvfs2_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VDVFS2_sel] value=%d, val=%d \n", volt, val);
}

void dct_pmic_MT6332_VDRAM_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VDRAM_sel] value=%d \n", volt);

    if (volt > 1300000 || volt < 700000)
        return;
    ...
    mt6332_upmu_set_vdram_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VDRAM_sel] value=%d, val=%d \n", volt, val);
}


void dct_pmic_MT6332_VPA_sel(kal_uint32 volt)
{
    int val = 0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VPA_sel] value=%d \n", volt);

    if (volt > 3400000 || volt < 500000)
        return;

    val = (volt - 500000) / 50000;  

    if (val > 0x3A)  
	val = 0x3A;

    mt6332_upmu_set_vpa_vosel(val);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[dct_pmic_MT6332_VPA_sel] value=%d, val=%d \n", volt, val);
}
...
void pmic_ldo_enable(MT65XX_POWER powerId, kal_bool powerEnable)
{
    ...
    else if(powerId == MT6331_POWER_BUCK_VDVFS11)	{ dct_pmic_VDVFS11_enable(powerEnable); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS12)	{ dct_pmic_VDVFS12_enable(powerEnable); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS13)	{ dct_pmic_VDVFS13_enable(powerEnable); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS14)	{ dct_pmic_VDVFS14_enable(powerEnable); }
    else if(powerId == MT6331_POWER_BUCK_VCORE2)	{ dct_pmic_VCORE2_enable(powerEnable); }
    else if(powerId == MT6331_POWER_BUCK_VIO18)		{ dct_pmic_VIO18_enable(powerEnable); }
    else if(powerId == MT6332_POWER_BUCK_VDVFS2)	{ dct_pmic_MT6332_VDVFS2_enable(powerEnable); }
    else if(powerId == MT6332_POWER_BUCK_VDRAM)		{ dct_pmic_MT6332_VDRAM_enable(powerEnable); }
    else if(powerId == MT6332_POWER_BUCK_VRF18_1)	{ dct_pmic_MT6332_VRF1_enable(powerEnable); }
    else if(powerId == MT6332_POWER_BUCK_VRF18_2)	{ dct_pmic_MT6332_VRF2_enable(powerEnable); }
    else if(powerId == MT6332_POWER_BUCK_VPA)		{ dct_pmic_MT6332_VPA_enable(powerEnable); }
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    ...
    else if(powerId == MT6331_POWER_BUCK_VDVFS11)	{ dct_pmic_VDVFS11_sel(powerVolt); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS12)	{ dct_pmic_VDVFS12_sel(powerVolt); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS13)	{ dct_pmic_VDVFS13_sel(powerVolt); }
    else if(powerId == MT6331_POWER_BUCK_VDVFS14)	{ dct_pmic_VDVFS14_sel(powerVolt); }
    else if(powerId == MT6331_POWER_BUCK_VCORE2)	{ dct_pmic_VCORE2_sel(powerVolt); }
    else if(powerId == MT6332_POWER_BUCK_VDVFS2)	{ dct_pmic_MT6332_VDVFS2_sel(powerVolt); }
    else if(powerId == MT6332_POWER_BUCK_VDRAM)         { dct_pmic_MT6332_VDRAM_sel(powerVolt); }
    else if(powerId == MT6332_POWER_BUCK_VPA)           { dct_pmic_MT6332_VPA_sel(powerVolt); }
...
int pmic_ldo_get_status(int powerId)
{
	...
	if(     powerId == MT6331_POWER_LDO_VTCXO1)        { show_LDO_VTCXO1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VTCXO2)        { show_LDO_VTCXO2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VAUD32)        { show_LDO_VAUD32_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VAUXA32)       { show_LDO_VAUXA32_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAMA)         { show_LDO_VCAMA_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMCH)          { show_LDO_VMCH_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VEMC33)        { show_LDO_VEMC33_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VIO28)         { show_LDO_VIO28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMC)           { show_LDO_VMC_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAM_AF)       { show_LDO_VCAM_AF_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP1)          { show_LDO_VGP1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP4)          { show_LDO_VGP4_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSIM1)         { show_LDO_VSIM1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSIM2)         { show_LDO_VSIM2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VFBB)          { show_LDO_VFBB_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VRTC)          { show_LDO_VRTC_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMIPI)         { show_LDO_VMIPI_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VIBR)          { show_LDO_VIBR_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VDIG18)        { show_LDO_32_VDIG18_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAMD)         { show_LDO_VCAMD_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VUSB10)        { show_LDO_VUSB10_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAM_IO)       { show_LDO_VCAM_IO_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSRAM_DVFS1)   { show_LDO_VSRAM_DVFS1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP2)          { show_LDO_VGP2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP3)          { show_LDO_VGP3_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VBIASN)        { show_LDO_VBIASN_STATUS(NULL, NULL, vreg_en_stat); }

	else if(powerId == MT6332_POWER_LDO_VBIF28)        { show_LDO_VBIF28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VAUXB32)       { show_LDO_VAUXB32_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VUSB33)        { show_LDO_VUSB33_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VDIG18)        { show_LDO_32_VDIG18_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VSRAM_DVFS2)   { show_LDO_VSRAM_DVFS2_STATUS(NULL, NULL, vreg_en_stat); }

	else if(powerId == MT6331_POWER_BUCK_VDVFS11)	{ show_BUCK_VDVFS11_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS12)   { show_BUCK_VDVFS12_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS13)   { show_BUCK_VDVFS13_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS14)   { show_BUCK_VDVFS14_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VCORE2)	{ show_BUCK_VCORE2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VIO18)	{ show_BUCK_VIO18_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VDVFS2)	{ show_BUCK_VDVFS2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VDRAM)	{ show_BUCK_VDRAM_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VRF18_1)   { show_BUCK_VRF1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VRF18_2)   { show_BUCK_VRF2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VPA)	{ show_BUCK_VPA_STATUS(NULL, NULL, vreg_en_stat); }
...
int pmic_ldo_get_voltage(int powerId)
{
	...
	if(     powerId == MT6331_POWER_LDO_VTCXO1)        { show_LDO_VTCXO1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VTCXO2)        { show_LDO_VTCXO2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VAUD32)        { show_LDO_VAUD32_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VAUXA32)       { show_LDO_VAUXA32_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAMA)         { show_LDO_VCAMA_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMCH)          { show_LDO_VMCH_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VEMC33)        { show_LDO_VEMC33_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VIO28)         { show_LDO_VIO28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMC)           { show_LDO_VMC_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAM_AF)       { show_LDO_VCAM_AF_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP1)          { show_LDO_VGP1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP4)          { show_LDO_VGP4_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSIM1)         { show_LDO_VSIM1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSIM2)         { show_LDO_VSIM2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VFBB)          { show_LDO_VFBB_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VRTC)          { show_LDO_VRTC_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VMIPI)         { show_LDO_VMIPI_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VIBR)          { show_LDO_VIBR_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VDIG18)        { show_LDO_32_VDIG18_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAMD)         { show_LDO_VCAMD_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VUSB10)        { show_LDO_VUSB10_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VCAM_IO)       { show_LDO_VCAM_IO_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VSRAM_DVFS1)   { show_LDO_VSRAM_DVFS1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP2)          { show_LDO_VGP2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VGP3)          { show_LDO_VGP3_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_LDO_VBIASN)        { show_LDO_VBIASN_VOLTAGE(NULL, NULL, vreg_en_stat); }

	else if(powerId == MT6332_POWER_LDO_VBIF28)        { show_LDO_VBIF28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VAUXB32)       { show_LDO_VAUXB32_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VUSB33)        { show_LDO_VUSB33_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VDIG18)        { show_LDO_32_VDIG18_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_LDO_VSRAM_DVFS2)   { show_LDO_VSRAM_DVFS2_VOLTAGE(NULL, NULL, vreg_en_stat); }

	else if(powerId == MT6331_POWER_BUCK_VDVFS11)	{ show_BUCK_VDVFS11_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS12)   { show_BUCK_VDVFS12_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS13)   { show_BUCK_VDVFS13_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VDVFS14)   { show_BUCK_VDVFS14_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VCORE2)	{ show_BUCK_VCORE2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6331_POWER_BUCK_VIO18)	{ show_BUCK_VIO18_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VDVFS2)	{ show_BUCK_VDVFS2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VDRAM)	{ show_BUCK_VDRAM_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VRF18_1)   { show_BUCK_VRF1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VRF18_2)   { show_BUCK_VRF2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6332_POWER_BUCK_VPA)	{ show_BUCK_VPA_VOLTAGE(NULL, NULL, vreg_en_stat); }
...
static ssize_t store_pmic_dvt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        ...
        #else
        printk("[store_pmic_dvt] no define CONFIG_MTK_PMIC_DVT_SUPPORT\n");
        #endif
    }    
    return size;
}
...
{    
    mt_set_gpio_mode(g_vproc_vsel_gpio_number,0); 
    mt_set_gpio_dir(g_vproc_vsel_gpio_number,1);  
    mt_set_gpio_out(g_vproc_vsel_gpio_number,val);
}

void ext_buck_vproc_en(int val)
{    
    mt_set_gpio_mode(g_vproc_en_gpio_number,0); 
    mt_set_gpio_dir(g_vproc_en_gpio_number,1);  
...
void pmic_dig_reset(void)
{
    ...
    ret_val=pmic_config_interface(MT6331_TOP_RST_MISC_CLR, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6331_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
    printk("[pmic_dig_reset] Reg[0x%x]=0x%x\n", MT6331_TOP_RST_MISC, upmu_get_reg_value(MT6331_TOP_RST_MISC));    
    ret_val=pmic_config_interface(MT6332_TOP_RST_MISC_CLR, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6332_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
...
void pmic_full_reset(void)
{
    ...
    ret_val=pmic_config_interface(MT6331_TOP_RST_MISC_SET, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6331_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
    printk("[pmic_full_reset] Reg[0x%x]=0x%x\n", MT6331_TOP_RST_MISC, upmu_get_reg_value(MT6331_TOP_RST_MISC));    
    ret_val=pmic_config_interface(MT6332_TOP_RST_MISC_SET, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6332_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
    printk("[pmic_full_reset] Reg[0x%x]=0x%x\n", MT6332_TOP_RST_MISC, upmu_get_reg_value(MT6332_TOP_RST_MISC));
}
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    if(mt6331_chip_version >= PMIC6331_E2_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x106,0x1,0x1,4); 
        ret = pmic_config_interface(0x106,0x1,0x1,5); 
        ret = pmic_config_interface(0x106,0x1,0x1,6); 
        ret = pmic_config_interface(0x124,0x1,0x1,0); 
        ret = pmic_config_interface(0x124,0x1,0x1,2); 
        ret = pmic_config_interface(0x124,0x1,0x1,3); 
        ret = pmic_config_interface(0x13E,0x1,0x1,2); 
        ret = pmic_config_interface(0x13E,0x1,0x1,3); 
        ret = pmic_config_interface(0x144,0x1,0x1,6); 
        ret = pmic_config_interface(0x14A,0x1,0x1,10); 
        ret = pmic_config_interface(0x150,0x1,0x1,4); 
        ret = pmic_config_interface(0x184,0x0,0x1,15); 
        ...
        ret = pmic_config_interface(0x24C,0x44,0x7F,0); 
        
        ret = pmic_config_interface(0x244,0x1,0x1,1); 
        ret = pmic_config_interface(0x250,0x0,0x7F,0); 
        ret = pmic_config_interface(0x25A,0x3,0x3,0); 
        ret = pmic_config_interface(0x25A,0x1,0x3,4); 
        ret = pmic_config_interface(0x25A,0x1,0x1,8); 
        ret = pmic_config_interface(0x262,0x1,0x1,0); 
        ret = pmic_config_interface(0x264,0x44,0x7F,0); 
        ret = pmic_config_interface(0x266,0x0,0x7F,0); 
        ret = pmic_config_interface(0x26A,0x5,0x7F,0); 
        ret = pmic_config_interface(0x26A,0x5,0x7F,8); 
        ret = pmic_config_interface(0x282,0x5,0x7F,0); 
        ret = pmic_config_interface(0x282,0x5,0x7F,8); 
        ret = pmic_config_interface(0x2A6,0x1,0x1,1); 
        ret = pmic_config_interface(0x2AC,0x17,0x7F,0); 
        ret = pmic_config_interface(0x2AC,0x1,0x1,7); 
        ret = pmic_config_interface(0x2AC,0x5,0x7F,8); 
        ret = pmic_config_interface(0x2AC,0x1,0x1,15); 
        ret = pmic_config_interface(0x2B2,0x0,0x7F,0); 
        ret = pmic_config_interface(0x2BC,0x3,0x3,0); 
        ret = pmic_config_interface(0x2BC,0x1,0x3,4); 
        ret = pmic_config_interface(0x2BC,0x1,0x1,8); 
        ret = pmic_config_interface(0x2D6,0x5,0x7F,0); 
        ret = pmic_config_interface(0x2D6,0x5,0x7F,8); 
        ret = pmic_config_interface(0x314,0x5,0x7F,0); 
        ret = pmic_config_interface(0x314,0x5,0x7F,8); 
        ret = pmic_config_interface(0x33E,0x5,0x7F,0); 
        ret = pmic_config_interface(0x33E,0x5,0x7F,8); 
        ret = pmic_config_interface(0x368,0x17,0x7F,0); 
        ret = pmic_config_interface(0x368,0x1,0x1,7); 
        ret = pmic_config_interface(0x368,0x5,0x7F,8); 
        ret = pmic_config_interface(0x368,0x1,0x1,15); 
        ret = pmic_config_interface(0x36A,0x44,0x7F,0); 
        
        ret = pmic_config_interface(0x362,0x1,0x1,1); 
        ret = pmic_config_interface(0x36E,0x0,0x7F,0); 
        ret = pmic_config_interface(0x378,0x3,0x3,0); 
        ret = pmic_config_interface(0x378,0x1,0x3,4); 
        ret = pmic_config_interface(0x378,0x1,0x1,8); 
        ret = pmic_config_interface(0x3A4,0x1,0x1,8); 
        ret = pmic_config_interface(0x502,0x1,0x1,11); 
        ret = pmic_config_interface(0x502,0x0,0x3,13); 
        ret = pmic_config_interface(0x504,0x1,0x1,11); 
        ret = pmic_config_interface(0x504,0x1,0x3,13); 
        ret = pmic_config_interface(0x506,0x1,0x1,0); 
        ret = pmic_config_interface(0x506,0x0,0x3,5); 
        ret = pmic_config_interface(0x506,0x0,0x3,13); 
        ret = pmic_config_interface(0x508,0x1,0x1,0); 
        ret = pmic_config_interface(0x508,0x0,0x3,5); 
        ret = pmic_config_interface(0x50E,0x0,0x3,5); 
        ret = pmic_config_interface(0x524,0x0,0x1,11); 
        ret = pmic_config_interface(0x534,0x44,0x7F,9); 
        ret = pmic_config_interface(0x538,0x0,0x1,2); 
        ret = pmic_config_interface(0x54A,0x1,0x1,0); 
        ret = pmic_config_interface(0x54A,0x0,0x3,5); 
        ret = pmic_config_interface(0x552,0x0,0x3,5); 
        ret = pmic_config_interface(0x73E,0x1,0x3,0); 
        ret = pmic_config_interface(0x73E,0x1,0x3,2); 
        ret = pmic_config_interface(0x73E,0x1,0x3,4); 
        ret = pmic_config_interface(0x73E,0x1,0x3,6); 
        ret = pmic_config_interface(0x73E,0x1,0x3,8); 
        ret = pmic_config_interface(0x73E,0x1,0x3,10); 
        ret = pmic_config_interface(0x73E,0x1,0x3,12); 
        ret = pmic_config_interface(0x73E,0x2,0x3,14); 
        ret = pmic_config_interface(0x740,0x1,0x3,0); 
        ret = pmic_config_interface(0x740,0x1,0x3,2); 
        ret = pmic_config_interface(0x740,0x1,0x3,4); 
        ret = pmic_config_interface(0x740,0x1,0x3,6); 
        ret = pmic_config_interface(0x6024,0x0006,0xFFFF,0); 
    ...
    else if(mt6331_chip_version >= PMIC6331_E1_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x13E,0x1,0x1,2); 
        ret = pmic_config_interface(0x13E,0x1,0x1,3); 
        ret = pmic_config_interface(0x144,0x0,0x1,2); 
        ret = pmic_config_interface(0x144,0x1,0x1,6); 
        ret = pmic_config_interface(0x14A,0x1,0x1,10); 
        ret = pmic_config_interface(0x150,0x1,0x1,4); 
        ret = pmic_config_interface(0x184,0x0,0x1,15); 
        ret = pmic_config_interface(0x244,0x0,0x1,0); 
        ret = pmic_config_interface(0x244,0x1,0x1,1); 
        ret = pmic_config_interface(0x24A,0x23,0x7F,0); 
        ret = pmic_config_interface(0x24A,0x1,0x1,7); 
        ret = pmic_config_interface(0x24A,0x6,0x7F,8); 
        ret = pmic_config_interface(0x24A,0x1,0x1,15); 
        ret = pmic_config_interface(0x24C,0x44,0x7F,0); 
        ...
        ret = pmic_config_interface(0x282,0x5,0x7F,0); 
        ret = pmic_config_interface(0x282,0x5,0x7F,8); 
        ret = pmic_config_interface(0x2A6,0x0,0x1,0); 
        ret = pmic_config_interface(0x2A6,0x1,0x1,1); 
        ret = pmic_config_interface(0x2AC,0x23,0x7F,0); 
        ret = pmic_config_interface(0x2AC,0x1,0x1,7); 
        ret = pmic_config_interface(0x2AC,0x6,0x7F,8); 
        ret = pmic_config_interface(0x2AC,0x1,0x1,15); 
        ret = pmic_config_interface(0x2B2,0x0,0x7F,0); 
        ret = pmic_config_interface(0x2BC,0x3,0x3,0); 
        ret = pmic_config_interface(0x2BC,0x1,0x3,4); 
        ...
        ret = pmic_config_interface(0x33E,0x5,0x7F,8); 
        ret = pmic_config_interface(0x362,0x1,0x1,1); 
        ret = pmic_config_interface(0x368,0x23,0x7F,0); 
        ret = pmic_config_interface(0x368,0x1,0x1,7); 
        ret = pmic_config_interface(0x368,0x8,0x7F,8); 
        ret = pmic_config_interface(0x368,0x1,0x1,15); 
        ret = pmic_config_interface(0x36A,0x44,0x7F,0); 
        ...
        ret = pmic_config_interface(0x378,0x1,0x3,4); 
        ret = pmic_config_interface(0x378,0x1,0x1,8); 
        ret = pmic_config_interface(0x394,0x5,0x7F,0); 
        ret = pmic_config_interface(0x394,0x5,0x7F,8); 
        ret = pmic_config_interface(0x3A4,0x1,0x1,8); 
        ret = pmic_config_interface(0x502,0x1,0x1,11); 
        ret = pmic_config_interface(0x502,0x0,0x3,13); 
        ret = pmic_config_interface(0x504,0x1,0x1,11); 
    ...
    if(mt6332_chip_version >= PMIC6332_E3_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x8004,0x1,0x1,4); 
        ret = pmic_config_interface(0x8004,0x1,0x1,5); 
        ret = pmic_config_interface(0x8004,0x1,0x1,6); 
        ret = pmic_config_interface(0x8004,0x1,0x1,7); 
        ret = pmic_config_interface(0x801A,0x1,0x1,0); 
        ret = pmic_config_interface(0x8094,0x1,0x1,9); 
        ret = pmic_config_interface(0x8424,0x1,0x1,0); 
        ret = pmic_config_interface(0x8424,0x1,0x1,1); 
        ret = pmic_config_interface(0x8424,0x1,0x1,2); 
        ret = pmic_config_interface(0x8426,0x4,0x7F,0); 
        ret = pmic_config_interface(0x8426,0x8,0x7F,8); 
        ret = pmic_config_interface(0x8428,0x25,0x7F,0); 
        ret = pmic_config_interface(0x8428,0x7F,0x7F,8); 
        ret = pmic_config_interface(0x843E,0x4,0x7,0); 
        ret = pmic_config_interface(0x8442,0x3,0x3,0); 
        ret = pmic_config_interface(0x845C,0x1,0x1,8); 
        ret = pmic_config_interface(0x846E,0x3,0x3,0); 
        ret = pmic_config_interface(0x8472,0x0,0x1,0); 
        ret = pmic_config_interface(0x8472,0x1,0x1,1); 
        ret = pmic_config_interface(0x8478,0x17,0x7F,0); 
        ret = pmic_config_interface(0x8478,0x1,0x1,7); 
        ret = pmic_config_interface(0x8478,0x8,0x7F,8); 
        ret = pmic_config_interface(0x8478,0x1,0x1,15); 
        ret = pmic_config_interface(0x847E,0x0,0x7F,0); 
        ret = pmic_config_interface(0x8488,0x3,0x3,0); 
        ret = pmic_config_interface(0x8488,0x1,0x3,4); 
        ret = pmic_config_interface(0x8488,0x1,0x1,8); 
        ret = pmic_config_interface(0x8490,0x1,0x1,0); 
        ret = pmic_config_interface(0x8494,0x0,0x7F,0); 
        ret = pmic_config_interface(0x8496,0x17,0x7F,0); 
        ret = pmic_config_interface(0x8496,0x1,0x1,7); 
        ret = pmic_config_interface(0x8496,0x8,0x7F,8); 
        ret = pmic_config_interface(0x8496,0x1,0x1,15); 
        ret = pmic_config_interface(0x8498,0x1,0x1,8); 
        ret = pmic_config_interface(0x84AA,0x1,0x1,0); 
        ret = pmic_config_interface(0x84B2,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84B4,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84D6,0x1,0x1,0); 
        ret = pmic_config_interface(0x84D8,0x1,0x3,0); 
        ret = pmic_config_interface(0x84DE,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84E0,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84FC,0x2,0x3,6); 
        ret = pmic_config_interface(0x851C,0x1,0x3,0); 
        ret = pmic_config_interface(0x851E,0x1,0x3,0); 
        ret = pmic_config_interface(0x8526,0x3,0x3,4); 
        ret = pmic_config_interface(0x8868,0x0,0x1,13); 
        ret = pmic_config_interface(0x8868,0x0,0x1,14); 
        ret = pmic_config_interface(0x8868,0x0,0x1,15); 
        ret = pmic_config_interface(0x886E,0x1,0x3,4); 
        ret = pmic_config_interface(0x886E,0x1,0x3,6); 
        ret = pmic_config_interface(0x886E,0x1,0x3,8); 
        ret = pmic_config_interface(0x886E,0x1,0x3,10); 
        ret = pmic_config_interface(0x886E,0x1,0x3,12); 
        ret = pmic_config_interface(0x886E,0x2,0x3,14); 
        ret = pmic_config_interface(0x8870,0x1,0x3,0); 
        ret = pmic_config_interface(0x8870,0x1,0x3,2); 
        ret = pmic_config_interface(0x8870,0x1,0x3,4); 
        ret = pmic_config_interface(0x8870,0x1,0x3,6); 
        ret = pmic_config_interface(0x8C06,0x1,0x1,5); 
        ret = pmic_config_interface(0x8C0A,0x1,0x1,4); 
        ret = pmic_config_interface(0x8C0A,0x1,0x1,5); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,8); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,9); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,10); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,11); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,12); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,13); 
        ret = pmic_config_interface(0x8C12,0x1,0x1,14); 
        ret = pmic_config_interface(0x8C16,0x1,0x7F,8); 
        ret = pmic_config_interface(0x8C18,0x1,0x1,7); 
        ret = pmic_config_interface(0x8C1A,0x1,0x1,0); 
        ret = pmic_config_interface(0x8C1A,0x1,0x1,1); 
        ret = pmic_config_interface(0x8C20,0x0,0x1,11); 
        ret = pmic_config_interface(0x8C40,0x1,0x1,8); 
        ret = pmic_config_interface(0x8C44,0x24,0xFFFF,0); 
        ret = pmic_config_interface(0x8C46,0x14,0xFF,0); 
        ret = pmic_config_interface(0x8C48,0xFF,0xFF,8); 
        ret = pmic_config_interface(0x8CB6,0x1,0x1,0); 
        ret = pmic_config_interface(0x8CB6,0x0,0x3,5); 
        ret = pmic_config_interface(0x8CB8,0x1,0x1,11); 
        ret = pmic_config_interface(0x8CB8,0x0,0x3,13); 
        ret = pmic_config_interface(0x8CBA,0x1,0x1,0); 
        ret = pmic_config_interface(0x8CBA,0x0,0x3,5); 
        ret = pmic_config_interface(0x8CBA,0x0,0x1,10); 
        ret = pmic_config_interface(0x8CBC,0x0,0x1,11); 
        ret = pmic_config_interface(0x8CD4,0xAE8,0x3FFF,0); 
        ret = pmic_config_interface(0xE024,0x0003,0xFFFF,0); 
    ...
    else if(mt6332_chip_version >= PMIC6332_E1_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x8004,0x1,0x1,3); 
        ret = pmic_config_interface(0x8004,0x1,0x1,4); 
        ret = pmic_config_interface(0x8004,0x1,0x1,5); 
        ret = pmic_config_interface(0x8004,0x1,0x1,6); 
        ret = pmic_config_interface(0x801A,0x1,0x1,0); 
        ret = pmic_config_interface(0x8094,0x1,0x1,9); 
        ret = pmic_config_interface(0x80A0,0x0,0x1,2); 
        ret = pmic_config_interface(0x80B2,0x1,0x1,4); 
        ret = pmic_config_interface(0x8424,0x1,0x1,0); 
        ret = pmic_config_interface(0x8424,0x1,0x1,1); 
        ret = pmic_config_interface(0x8424,0x1,0x1,2); 
        ret = pmic_config_interface(0x8426,0x4,0x7F,0); 
        ...
        ret = pmic_config_interface(0x8428,0x7F,0x7F,8); 
        ret = pmic_config_interface(0x843E,0x4,0x7,0); 
        ret = pmic_config_interface(0x843E,0x0,0xF,8); 
        ret = pmic_config_interface(0x8442,0x3,0x3,0); 
        ret = pmic_config_interface(0x844C,0x5,0x7F,0); 
        ret = pmic_config_interface(0x844C,0x5,0x7F,8); 
        ret = pmic_config_interface(0x845C,0x1,0x1,8); 
        ret = pmic_config_interface(0x8472,0x0,0x1,0); 
        ret = pmic_config_interface(0x8472,0x1,0x1,1); 
        ret = pmic_config_interface(0x8478,0x23,0x7F,0); 
        ret = pmic_config_interface(0x8478,0x1,0x1,7); 
        ret = pmic_config_interface(0x8478,0x8,0x7F,8); 
        ret = pmic_config_interface(0x8478,0x1,0x1,15); 
        ret = pmic_config_interface(0x847E,0x0,0x7F,0); 
        ...
        ret = pmic_config_interface(0x8498,0x1,0x1,8); 
        ret = pmic_config_interface(0x84AA,0x1,0x1,0); 
        ret = pmic_config_interface(0x84B0,0x5,0x7F,0); 
        ret = pmic_config_interface(0x84B0,0x5,0x7F,8); 
        ret = pmic_config_interface(0x84B2,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84B4,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84D6,0x1,0x1,0); 
        ret = pmic_config_interface(0x84D8,0x1,0x3,0); 
        ret = pmic_config_interface(0x84DC,0x5,0x7F,0); 
        ret = pmic_config_interface(0x84DC,0x5,0x7F,8); 
        ret = pmic_config_interface(0x84DE,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84E0,0x53,0x7F,0); 
        ret = pmic_config_interface(0x84FC,0x2,0x3,6); 
        ret = pmic_config_interface(0x850A,0x1,0x7F,0); 
        ret = pmic_config_interface(0x850A,0x1,0x7F,8); 
        ret = pmic_config_interface(0x851C,0x1,0x3,0); 
        ret = pmic_config_interface(0x851E,0x1,0x3,0); 
        ret = pmic_config_interface(0x8526,0x3,0x3,4); 
        ret = pmic_config_interface(0x8540,0x5,0x7F,0); 
        ret = pmic_config_interface(0x8540,0x5,0x7F,8); 
        ret = pmic_config_interface(0x8868,0x0,0x1,13); 
        ret = pmic_config_interface(0x8868,0x0,0x1,14); 
        ret = pmic_config_interface(0x8868,0x0,0x1,15); 
        ret = pmic_config_interface(0x886E,0x1,0x3,4); 
        ...
        ret = pmic_config_interface(0x8CBA,0x1,0x1,0); 
        ret = pmic_config_interface(0x8CBA,0x0,0x3,5); 
        ret = pmic_config_interface(0x8CBA,0x1,0x1,10); 
        ret = pmic_config_interface(0x8CBC,0x1,0x1,0); 
        ret = pmic_config_interface(0x8CBC,0x0,0x1,11); 
        ret = pmic_config_interface(0x8CD4,0xAE8,0x3FFF,0); 
        ret = pmic_config_interface(0xE024,0x0003,0xFFFF,0); 
...
void PMIC_INIT_SETTING_V2(void)
{
    ...
    if(mt6331_chip_version >= PMIC6331_E1_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x33E,0x5,0x7F,0); 
        ret = pmic_config_interface(0x33E,0x5,0x7F,8); 
        ret = pmic_config_interface(0x35C,0x0,0x1,9); 
        ret = pmic_config_interface(0x366,0x0,0x1,0); 
        ret = pmic_config_interface(0x368,0x17,0x7F,0); 
        ret = pmic_config_interface(0x368,0x1,0x1,7); 
        ret = pmic_config_interface(0x368,0x5,0x7F,8); 
        ret = pmic_config_interface(0x368,0x1,0x1,15); 
        ret = pmic_config_interface(0x36A,0x44,0x7F,0); 
        ret = pmic_config_interface(0x36C,0x44,0x7F,0); 
        ret = pmic_config_interface(0x362,0x1,0x1,1); 
        ret = pmic_config_interface(0x36E,0x0,0x7F,0); 
        ret = pmic_config_interface(0x378,0x3,0x3,0); 
        ret = pmic_config_interface(0x378,0x1,0x3,4); 
        ...
        ret = pmic_config_interface(0x508,0x0,0x3,5); 
        ret = pmic_config_interface(0x50E,0x0,0x3,5); 
        ret = pmic_config_interface(0x524,0x0,0x1,10); 
        ret = pmic_config_interface(0x524,0x0,0x1,11); 
        ret = pmic_config_interface(0x52C,0x0,0x1,10); 
        ret = pmic_config_interface(0x534,0x44,0x7F,9); 
        ret = pmic_config_interface(0x538,0x0,0x1,2); 
        ret = pmic_config_interface(0x54A,0x1,0x1,0); 
        ret = pmic_config_interface(0x54A,0x0,0x3,5); 
    ...
    if(mt6332_chip_version >= PMIC6332_E1_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x801A,0x1,0x1,0); 
        ret = pmic_config_interface(0x8094,0x1,0x1,9); 
        ret = pmic_config_interface(0x8424,0x0,0x1,0); 
        ret = pmic_config_interface(0x8424,0x0,0x1,1); 
        ret = pmic_config_interface(0x8424,0x0,0x1,2); 
        ret = pmic_config_interface(0x8426,0x4,0x7F,0); 
        ret = pmic_config_interface(0x8426,0x8,0x7F,8); 
        ret = pmic_config_interface(0x8428,0x25,0x7F,0); 
        ret = pmic_config_interface(0x8428,0x7F,0x7F,8); 
...
static int pmic_mt_probe(struct platform_device *dev)
{
    ...
    pmic_config_interface(0x1E,0x0,0x1,11); 
    pmic_config_interface(0x8C20,0x0,0x1,11); 
    ...
    mt6331_upmu_set_rg_uvlo_vthl_vsys(0x3);
    mt6331_upmu_set_rg_uvlo_vthl(0x3);
...
static void pmic_early_suspend(struct early_suspend *h)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", " suspend set M3 off to zero Reg[0x%x]=0x%x\n", 0x806C, upmu_get_reg_value(0x806C));
#endif
...
static void pmic_early_resume(struct early_suspend *h)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", " resume set M3 off to zero Reg[0x%x]=0x%x\n", 0x806C, upmu_get_reg_value(0x806C));
#endif
}
