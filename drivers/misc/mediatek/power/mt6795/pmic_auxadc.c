...
int PMIC_IMM_GetOneChannelValue(upmu_adc_chl_list_enum dwChannel, int deCount, int trimd)
{
	...
	if (channel_num == 7 && chip_num == MT6332_CHIP) {	
		...
		if(mt6332_upmu_get_swcid()==PMIC6332_E1_CID_CODE) {
			...
			xlog_printk(ANDROID_LOG_DEFAULT, "Power/PMIC", "[PMIC_IMM_GetOneChannelValue] usb E2 \n");
...
#if 1		
		PMIC_IMM_RequestAuxadcChannel(channel_num, chip_num, user_num, CLEAR_REQ);		
		PMIC_IMM_RequestAuxadcChannel(channel_num, chip_num, user_num, SET_REQ);		
#else
		PMIC_IMM_RequestAuxadcChannel(channel_num, chip_num, user_num, ONLY_REQ);		
#endif
		udelay(1);
