...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
  ...
  static const unsigned int ultra_low_time      = 5; 
  static const unsigned int pre_ultra_low_time  = 7; 
  static const unsigned int pre_ultra_high_time = 8; 
                    ...
                    void * handle)  
{
    ...
    else
    {
        ...
        DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 496); 
