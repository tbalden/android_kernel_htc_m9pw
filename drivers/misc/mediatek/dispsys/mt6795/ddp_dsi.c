...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	hs_trail_m=1;
	hs_trail_n= (dsi_params->HS_TRAIL == 0) ? NS_TO_CYCLE(((hs_trail_m * 0x4 * ui) + 60), cycle_time) : dsi_params->HS_TRAIL;
	...
	timcon2.CLK_TRAIL	= ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time) : dsi_params->CLK_TRAIL);
...
#ifdef GPIO_LCM_RST2
	mt_set_gpio_mode(GPIO_LCM_RST2, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_LCM_RST2, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_LCM_RST2, value?GPIO_OUT_ONE:GPIO_OUT_ZERO);
#endif
#endif	
}
...
int ddp_dsi_switch_mode(DISP_MODULE_ENUM module, void *cmdq_handle, void* params)
{
	...
	if(mode == 0)
	{
		...
		DSI_OUTREG32(cmdq_handle, (unsigned long)(DSI_REG[i])+0x130, 0x00001521|(lcm_cmd.addr << 16)|(lcm_cmd.val[0]<< 24)); 
		...
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x3, 0x0); 
	...
	else
	{
		...
		DSI_MASKREG32(cmdq_handle, 0xF4020020, 0x1, 0x1); 
		...
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x3, 0x1); 
		DSI_MASKREG32(cmdq_handle, 0xF4020020, 0x1, 0x1); 
