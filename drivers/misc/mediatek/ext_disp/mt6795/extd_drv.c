...
#ifdef DISABLE_HDMI_LOW_POWER
static struct switch_dev hdmi_pm_switch_data;
#endif
...
static void hdmi_early_suspend(struct early_suspend *h)
{
    ...
    if(IS_HDMI_ON())
    {
        switch_set_state(&hdmi_pm_switch_data, 0);
    }
    return;
#endif
...
static void hdmi_late_resume(struct early_suspend *h)
{
    ...
    if(IS_HDMI_ON())
    {
        switch_set_state(&hdmi_pm_switch_data, 1);
    }
#endif
...
void hdmi_state_callback(HDMI_STATE state)
{
    ...
    switch (state)
    {
        case HDMI_STATE_NO_DEVICE:
        {
            ...
            switch_set_state(&hdmires_switch_data, 0);
#ifdef DISABLE_HDMI_LOW_POWER
            switch_set_state(&hdmi_pm_switch_data, 0xff);
#endif
            mmdvfs_mhl_enable(false); 
            break;
        }
...
#ifdef DISABLE_HDMI_LOW_POWER
    hdmi_pm_switch_data.name = "pm_hdmi";
    hdmi_pm_switch_data.index = 0;
    hdmi_pm_switch_data.state = 0xff;

    
    ret = switch_dev_register(&hdmi_pm_switch_data);
#endif
