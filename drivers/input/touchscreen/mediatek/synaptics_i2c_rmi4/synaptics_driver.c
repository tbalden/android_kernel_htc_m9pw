...
#ifndef CONFIG_OF
static struct i2c_board_info i2c_tpd __initdata = { I2C_BOARD_INFO("mtk-tpd", (0x20)) };
#endif
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		{
			ret =
			    request_irq(touch_irq, tpd_eint_handler, EINTF_TRIGGER_RISING,
			...
			ret =
			    request_irq(touch_irq, tpd_eint_handler, EINTF_TRIGGER_FALLING,
					"TOUCH_PANEL-eint", NULL);
...
#ifdef USE_REGULATOR_FRAMEWORK
	ts->core_reg = devm_regulator_get(&client->dev, "reg-vgp1");
	ts->io_reg = devm_regulator_get(&client->dev, "reg-vgp3");
