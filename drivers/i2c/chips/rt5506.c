...
int rt5506_headset_detect(void *private_data, int on)
{
	if (!rt5506Connect) {
		...
		pr_info("%s: rt5506Connect %d, on %d, reDetectOM %d\n",
			__func__, rt5506Connect, on, reDetectOM);
		return 0;
	}

	if(on) {

		pr_info("%s: headset in, rt55xx version 0x%x(dts 0x%x) reDetectOM %d ++\n",
			__func__, rt5506_query.rt55xx_version, pdata->rt55xx_version, reDetectOM);
...
static void hs_imp_detec_func(struct work_struct *work)
{
	...
	if(temp[0] & AMP_SENSE_READY) {
		...
		pr_info("rt5506 hs imp value 0x%x hsmode %d om 0x%x hsom %d r_channel 0x%x high_imp %d, rt55xx version 0x%x(dts 0x%x)\n",\
			temp[0] & 0xf, hsmode, om, hsom, r_channel, high_imp, rt5506_query.rt55xx_version, pdata->rt55xx_version);
	} else {
