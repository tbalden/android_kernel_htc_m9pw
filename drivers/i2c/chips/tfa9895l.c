...
static int tfa9895_i2c_write(char *txdata, int length)
{
	...
	if ((txdata[0] == 0x04) &&
		...
		((txdata[2] & 0x7) != 0x3))
	{
		pr_err("ta9895_l: (txdata[2] & 0x7) = 0x%x, force correct last bit: 0x%x\n",
					(txdata[2] & 0x7), txdata[2]);
		txdata[2] &= ~(0x7);
		txdata[2] |= 0x3;
		pr_err("ta9895_l: after correct txdata[2], last bit: 0x%x\n", txdata[2]);
	}
#endif
	if (length <= 8) {
...
int tfa9895l_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("%s: pull enable %d %d\n", __func__, mt_get_gpio_pull_enable(GPIO_I2C4_SDA_PIN), mt_get_gpio_pull_enable(GPIO_I2C4_SCA_PIN));
	mt_set_gpio_pull_select(GPIO_I2C4_SDA_PIN, GPIO_PULL_UP);
	mt_set_gpio_pull_select(GPIO_I2C4_SCA_PIN, GPIO_PULL_UP);
	pr_info("%s: pull select %d %d\n", __func__, mt_get_gpio_pull_select(GPIO_I2C4_SDA_PIN), mt_get_gpio_pull_select(GPIO_I2C4_SCA_PIN));
#endif
