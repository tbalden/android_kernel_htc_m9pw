...
s32 set_alloc_bitmap(struct super_block *sb, u32 clu)
{
	...
	if ((i >= p_fs->map_sectors) || (i < 0)) {
		printk(KERN_ERR "[exFAT] %s() error value! clu:%d, i:%d, b:%d (max of i:%u)\n"
				, __func__, clu, i, b, p_fs->map_sectors);
		return FFS_ERROR;
	}
...
u32 test_alloc_bitmap(struct super_block *sb, u32 clu)
{
	...
	if ((map_i >= p_fs->map_sectors) || (map_i < 0)) {
		printk(KERN_ERR "[exFAT] %s() error value! clu:%u, i:%d, b:%d (max of i:%u)\n"
				, __func__, clu, map_i, map_b, p_fs->map_sectors);
		return CLUSTER_32(~0);
	}
