...
void Afe_Set_Reg_I2S0_CLEAR()
{
    printk("+Afe_Set_Reg_I2S0_CLEAR: reset I2S0_FROM\n");
    I2S0_FROM = 0;
}

void Afe_Set_Reg_I2S0_CHECK(uint32 offset, uint32 value, uint32 mask, uint32 from)
{
    printk("+Afe_Set_Reg_I2S0_CHECK: value=0x%x, mask=0x%x, from=%d\n", value, mask, from);
    ...
    if (I2S0_FROM)
    {
        if (value == 0)
        {
            ...
            printk("Afe_Set_Reg_I2S0_CHECK: I2S0_FROM still have value, don't close I2S\n");
        }
        else
        {
            Afe_Set_Reg(offset, value, mask);
        }
    }
    else
    {
    ...
    printk("-Afe_Set_Reg_I2S0_CHECK: opened I2S0_FROM=0x%x\n", I2S0_FROM);
}
...
EXPORT_SYMBOL(Afe_Set_Reg_I2S0_CLEAR);
EXPORT_SYMBOL(Afe_Set_Reg_I2S0_CHECK);
