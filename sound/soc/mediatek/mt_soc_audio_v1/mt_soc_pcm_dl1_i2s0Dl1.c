...
static const char *Audio_I2S0_Value[] = {"Off", "On"};
...
static const struct soc_enum Audio_I2S0dl1_Enum[] =
{
    ...
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Audio_I2S0_Value), Audio_I2S0_Value),
};
...
Audio_I2S0_Switch_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	
	ucontrol->value.integer.value[0] = Afe_Get_Reg(AFE_I2S_CON);
	return 0;
}
...
Audio_I2S0_Switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int useSram;
	printk("%s() %d\n", __func__, (int) ucontrol->value.integer.value[0]);
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_I2S0_Value))
	{
		printk("return -EINVAL\n");
		return -EINVAL;
	}
...
static const struct snd_kcontrol_new Audio_snd_I2S0dl1_controls[] =
{
    ...
    SOC_ENUM_EXT("Audio_I2S0_enable", Audio_I2S0dl1_Enum[5], Audio_I2S0_Switch_get, Audio_I2S0_Switch_set),
};
